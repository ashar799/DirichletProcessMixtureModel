### This is a toy example in which molecular data doesn't show clustering
### The time data also  shows a clustering
### Let's keep the Dimension to be 5
...
for ( i in 1:F){
  time.noise.list[[i]] <- rnorm(as.integer(N * p.dist[i]), mean = 2, sd = 0.1 )
}
...
plotsurv <- survfit(surv.ob ~ c.final)
list.betahat <- list(0)
...
pdf("featureselection.pdf")
heatmap.2(t(as.matrix(heatmapdata)),dendrogram="none", col =cm.colors(180), margins=c(5,5), main = "\n Posterior Probs \n for Selection \n ", cexCol = 0.85, cexRow = 0.7, Rowv = FALSE, key =TRUE)
dev.off()
