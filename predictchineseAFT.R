### This function takes the posterior parameters and updates the class membership of the new points
#### The fundamental assumption is that EACH NEW TEST POINT IS CONDITIONALLY INDEPENDENT on the OTHER POINTS
##### For the regression part, these individual points would be scaled by the mean and variance of the corresponding cluster
## The final output is just the posterior probabilities of cluster assigments
### Just need to collect samples for S

predictchineseAFT = function(Y.new, time.new, N.new, c.list ,Y, mu.list, S, alpha, beta0.list, betahat.list, sigma2.list, lambda2.list, tau2.list, K, D, N){
