posteriorchineseAFT = function(c,Y,mu,S,alpha,That, beta0, betahat, sigma2, lambda2, tau2, K, epsilon, W, beta, ro,D, r, si, Time,N, sig2.dat) {
 ...
 for(l in 1:N)  {
    ...
    if(length(which(cminus==temp))==0)  
    {
      ...
      S[active[kminus+1],1:D,1:D]  <- priortwo$Sigma[1,1:D,1:D]  
      ...
      S[active[kminus+2],1:D,1:D]  <- priorthree$Sigma[1,1:D,1:D]  
    ...
    for (j in 1:kminus) {
      ...
      posterior[j] <- g[active[j]] /(N-1+alpha) * dmvnorm(as.vector(t(Y[l,1:D])), mean = mu[active[j],1:D], sigma= solve(S[active[j],1:D,1:D])) *  dnorm(x = That[l], mean = beta0[active[j]] + betahat[active[j],1:D] %*% as.vector(t(Ytemp[l,1:D])), sd = sqrt(sigma2[active[j]]) )
    }
    
    
      posterior[kminus+1] <- (0.5 * alpha) /(N-1+alpha) * dmvnorm(as.vector(t(Y[l,1:D])), mean = mu[active[kminus+1],1:D], sigma= solve(S[active[kminus+1],1:D,1:D])) *  dnorm(x = That[l], mean = beta0[active[kminus+1]] + betahat[active[kminus+1],1:D] %*% as.vector(t(Ytemp[l,1:D])), sd = sqrt(sigma2[active[kminus+1]]) )
    
      posterior[kminus+2] <- (0.5 * alpha) /(N-1+alpha) * dmvnorm(as.vector(t(Y[l,1:D])), mean = mu[active[kminus+2],1:D], sigma= solve(S[active[kminus+2],1:D,1:D])) *  dnorm(x = That[l], mean = beta0[active[kminus+2]] + betahat[active[kminus+2],1:D] %*% as.vector(t(Ytemp[l,1:D])), sd = sqrt(sigma2[active[kminus+2]]) )
