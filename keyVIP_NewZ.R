...
load("/home/bit/ashar/ExpressionSets/ONE_VIEW/VIPdataset/DPMM-FactorAnalysis/DataVIPOWNsignaturePLUSMeanSamplesZ.RData")
...
heatmap.2(x = t(Y.order), Rowv =TRUE ,Colv = FALSE, dendrogram = c("row"), scale ="row",ColSideColors =c("red","black")[c.ultimate], labRow = colnames(Y), labCol = rownames(Y.order), main = ' \n DPMM clustering of VIP patients \n Centre Samples (39 Z) \n 60 Gene DPMM signature', col =hmcols, cexRow =0.40, trace ="none", key.title = "Color Key")
legend("topright", legend = c("Cluster 1","Cluster 2"),fill = c("Red","Black"), cex = 0.4)
p1
dev.off()

pdf("Z-VIPsurvival-curves.pdf")
plot(surv.fit, main = 'PF Survival Curves for VIP patients \n p = 0.01 \n Clustering based on Z samples', xlab= 'Mean PFS Cluster 1: 637d \n Mean PFS Cluster2: 278d')
dev.off()

save(list =ls(), file = 'ZVIPDPMM.RData')
