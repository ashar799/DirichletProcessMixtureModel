#### This file runs our model on the VIP data ##############################################
...
rm(list = ls())
load('/home/bit/ashar/ExpressionSets/VIPdataset/expressionsets.RData')

set <- ex[[1]]
dat <- (as.data.frame(set))

survi <- pData(set)$PFS
surv.days <- 30*as.numeric(as.matrix(survi))
index <- !is.na(surv.days)

data <- dat[index,]
survival <- surv.days[index]
